...
local INFO_PANEL_EQUIP_BUTTON = GET(INFO_PANEL, "EquipButton")
...
function Init()
    ...
    -- Setup equip button
    INFO_PANEL_EQUIP_BUTTON.clickedEvent:Connect(OnEquipButtonClicked)
...
function SetupLoadoutButtons()
    ...
    for setIndex, equipmentList in ipairs(loadoutList) do
        ...
        for equipmentIndex, equipmentData in ipairs(equipmentList) do
            ...
            -- Connect clicked event and set the button index
...
-- nil CheckEquipButton()
-- Check if there is a need to show equip button for the current selected equipment
function CheckEquipButton()
    if not currentButton then return end

    local currentLoadout = AEL.GetCurrentLoadout(LOCAL_PLAYER)

    if currentLoadout[currentSetIndex] == currentIndex then
        INFO_PANEL_EQUIP_BUTTON.isInteractable = false
    else
        INFO_PANEL_EQUIP_BUTTON.isInteractable = true
    end
end
...
-- nil OnEquipButtonClicked()
-- Send event to server when player equips an equipment
function OnEquipButtonClicked()
    if not currentButton then return end
